...
export default function Index() {
  ...
  const openFile = (uri: string) => {
    const filename = uri.split('/').pop()?.replace(/\.txt$/, '');
  ...
  const pickFile = async () => {
    ...
    console.error('Error picking or copying audio:', error);
  }
};
...
const styles = StyleSheet.create({
  ...
  text: { fontSize: 17, fontWeight: 'bold', color: '#555', marginLeft: 8 },

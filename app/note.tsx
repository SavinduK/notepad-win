...
export default function Index() {
    ...
    const { path } = useLocalSearchParams<{ path: string }>();//whole path

    const [isSharing, setIsSharing] = useState(false);

    const [uri, setUri] = useState('');//directory
    const [note, setNote] = useState('');//filename
    ...
    const downloadFile = async() =>{
         try {
    // Request permission for media library (needed on Android)
    const { status } = await MediaLibrary.requestPermissionsAsync();
    if (status !== 'granted') {
      console.log('Permission denied to save file to Downloads');
      return;
    }

    const fileContents = text;
    const filename = note || 'note.txt';
